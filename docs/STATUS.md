...
| `bionpu.cli` — `bionpu scan` (CPU path) | ✅ working end-to-end | Pure-numpy CRISPR off-target scan in `bionpu.scan`. Produces a canonical TSV that `bionpu verify crispr` can compare byte-equally. NGG PAM only; `--device npu` is v0.2 scope and exits with a clear pointer. |
| `bionpu.cli` — `bionpu basecall` / `bionpu bench` | ⚠️ v0.2 scope | Stubs that print a "v0.2 scope" message. The kernels they would invoke are extracted (and individually buildable via `make`); what's missing is the top-level Python that ties pod5 input → NPU dispatch → output → verify into one command. |

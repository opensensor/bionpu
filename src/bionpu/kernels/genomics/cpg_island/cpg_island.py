...
#
# cpg_island.py — IRON lowering for the v0 CpG island candidate kernel.
#
...
LAUNCH_CHUNKS = int(_os.environ.get("BIONPU_CPG_ISLAND_LAUNCH_CHUNKS", "4"))
if LAUNCH_CHUNKS not in (1, 2, 4, 8):
    raise ValueError(
        f"BIONPU_CPG_ISLAND_LAUNCH_CHUNKS={LAUNCH_CHUNKS} "
        "not in {1, 2, 4, 8}"
    )
N_TILES = LAUNCH_CHUNKS

SEQ_CHUNK_BYTES = 4096
SEQ_OVERLAP_BYTES = 52
HEADER_BYTES = 8
...
N_CHUNKS_PER_LAUNCH = int(_os.environ.get(
    "BIONPU_CPG_ISLAND_N_CHUNKS_PER_LAUNCH", "1"))
if N_CHUNKS_PER_LAUNCH not in (1, 2, 4, 8):
    raise ValueError(
        f"BIONPU_CPG_ISLAND_N_CHUNKS_PER_LAUNCH={N_CHUNKS_PER_LAUNCH} "
        "not in {1, 2, 4, 8}"
    )


def cpg_island(dev):
    """Build the IRON program for CpG-island candidate scanning."""
    ...
    partial_fifos = of_sparse.prod().join(
        ...
        names=[f"partial_cpg_{i}" for i in range(N_TILES)],
        depths=[1] * N_TILES,
    )

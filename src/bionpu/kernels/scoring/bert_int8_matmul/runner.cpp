...
constexpr int K = 768;
constexpr int N = 2;
constexpr size_t X_BYTES = static_cast<size_t>(M) * K;            // 36 096
constexpr size_t W_BYTES = static_cast<size_t>(N) * K;            //  1 536
constexpr size_t S_BYTES = static_cast<size_t>(N + 1) * sizeof(float);
constexpr size_t Y_BYTES = static_cast<size_t>(M) * N;            //     94
...
int main(int argc, char **argv) {
    std::string xclbin_path, insts_path, x_path, w_path, s_path, out_path;
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--s" && i + 1 < argc) s_path = argv[++i];
        else if (a == "--out" && i + 1 < argc) out_path = argv[++i];
        else usage(argv[0]);
    }
    if (xclbin_path.empty() || insts_path.empty() || x_path.empty() ||
        w_path.empty() || s_path.empty() || out_path.empty()) {
        usage(argv[0]);
    }

    // Load runtime artifacts.
    auto x_data = read_file<int8_t>(x_path, X_BYTES);
    auto w_data = read_file<int8_t>(w_path, W_BYTES);
    auto s_data = read_file<float>(s_path, S_BYTES);
    ...
    // Allocate device-shared buffers.
    ...
    auto bo_w = xrt::bo(device, W_BYTES, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
    auto bo_s = xrt::bo(device, S_BYTES, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(5));
    auto bo_y = xrt::bo(device, Y_BYTES, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(6));

    std::memcpy(bo_insts.map<uint32_t *>(), insts.data(),
                insts.size() * sizeof(uint32_t));
    std::memcpy(bo_x.map<int8_t *>(), x_data.data(), X_BYTES);
    std::memcpy(bo_w.map<int8_t *>(), w_data.data(), W_BYTES);
    std::memcpy(bo_s.map<float *>(), s_data.data(), S_BYTES);

    bo_insts.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    bo_x.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    bo_w.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    bo_s.sync(XCL_BO_SYNC_BO_TO_DEVICE);

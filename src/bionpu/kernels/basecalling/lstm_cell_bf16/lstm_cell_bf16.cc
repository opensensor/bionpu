...
// Tile-resident persistent state. All bf16. The kernel holds all gate
// accumulators in bf16 (storage) but accumulates the dot product in
// fp32 internally inside one chunk — bf16 storage avoids the AIE2P
// codegen issues seen when mixing per-gate fp32 buffers with bf16
// stores via small stack arrays.
static bfloat16 h_state[HIDDEN];
static bfloat16 c_state[HIDDEN];
// 4 gate accumulators, each HIDDEN bf16. Holds the running
// pre-nonlinearity sum z_g = W_ih·x + b_ih + W_hh·h + b_hh as bf16.
// Each chunk reads gate_acc[g][oc] (bf16), upgrades to fp32 for the
// dot-product accumulation, writes back as bf16.
static bfloat16 gate_acc[N_GATES][HIDDEN];
// Cached biases. Loaded from the first weight chunk's bias prefix on
// (t == 0, g == 0). N_GATES * 2 (ih, hh) * HIDDEN = 768 bf16.
...
// Vectorised hardware tanh. Uses an accfloat round-trip so the input
// goes to aie::tanh<bfloat16> as a vector<float> as the AIE2P kernels
// at silu.cc:39 / swiglu.cc:53 / gelu.cc:55 expect.
static inline aie::vector<bfloat16, VEC>
...
void dorado_fast_lstm_cell_bf16(bfloat16 *restrict x_t,
  ...
  for (int oc = 0; oc < HIDDEN; ++oc) {
    ...
    // Reduce the lane accumulator to a scalar.
    auto out_vec = acc.to_vector<float>();
    float sum = 0.0f;
    for (int k = 0; k < VEC; ++k) sum += out_vec.get(k);
    // Add to the running gate accumulator (bf16 storage).

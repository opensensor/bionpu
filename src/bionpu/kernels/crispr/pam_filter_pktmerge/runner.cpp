...
constexpr int EMIT_SLOT_RECORDS = 256;
...
constexpr int SPARSE_OUT_VOL = N_CHUNKS * EMIT_SLOT_BYTES;     // 131072
...
int main(int argc, char **argv) {
    ...
    auto windows = read_blob(args.windows_path, WINDOWS_IN_VOL);
    ...
    auto bo_guides = xrt::bo(device, GUIDES_VOL, XRT_BO_FLAGS_HOST_ONLY,
                              kernel.group_id(3));
    auto bo_windows = xrt::bo(device, WINDOWS_IN_VOL, XRT_BO_FLAGS_HOST_ONLY,
                               kernel.group_id(4));
    auto bo_out = xrt::bo(device, SPARSE_OUT_VOL, XRT_BO_FLAGS_HOST_ONLY,
                          kernel.group_id(5));
    ...
    std::memcpy(p_windows, windows.data(), WINDOWS_IN_VOL);

    auto *p_out = bo_out.map<uint8_t *>();
    std::memset(p_out, 0, SPARSE_OUT_VOL);
    ...
    write_blob(args.out_path, p_out, SPARSE_OUT_VOL);

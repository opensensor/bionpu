...
// runner_fused_batched.cpp — host runner for the batched fused linear
// projection. Wire format: bf16 input (n_chunks * L * 96), bf16 weight
// (24576; full slab, SHARED across batch), bf16 output (n_chunks * L *
// 256). The kernel itself is unchanged — chunk-outer batching is
// purely a host-amortisation trick: a single xclbin load + BO alloc
// + XRT submit covers n_chunks chunks of work.
...
constexpr int WB_LEN = OUT_DIM * HIDDEN; // 24576
...
Args parse(int argc, char **argv) {
  ...
  if (a.xclbin.empty() || a.instr.empty() || a.input_path.empty() ||
    ...
    throw std::runtime_error(
        ...
        "--output <path> [--seq L] [--n-chunks N]");
  if (a.n_chunks <= 0)
    throw std::runtime_error("--n-chunks must be > 0");
  return a;
}
...
int main(int argc, char **argv) {
  ...
  // Per-chunk geometry stays L * HIDDEN / L * OUT_DIM. The batched
  // wire format concatenates n_chunks copies of the per-chunk format.
  const size_t IN_TOTAL =
      static_cast<size_t>(args.n_chunks) * args.seq * HIDDEN;
  const size_t OUT_TOTAL =
      static_cast<size_t>(args.n_chunks) * args.seq * OUT_DIM;
  const size_t WB_TOTAL = static_cast<size_t>(WB_LEN);

  auto input = read_bf16(args.input_path, IN_TOTAL);
  auto wb = read_bf16(args.wb_path, WB_TOTAL);
  ...
  auto bo_weight = xrt::bo(device, WB_TOTAL * BYTES_PER_BF16,
  ...
  std::memcpy(bufWeight, wb.data(), WB_TOTAL * BYTES_PER_BF16);

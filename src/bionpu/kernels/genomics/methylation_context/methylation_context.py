...
#
# methylation_context.py — IRON lowering for methylation_context v0.
#
...
LAUNCH_CHUNKS = int(_os.environ.get(
    "BIONPU_METHYLATION_CONTEXT_LAUNCH_CHUNKS", "4"))
if LAUNCH_CHUNKS not in (1, 2, 4, 8):
    raise ValueError(
        f"BIONPU_METHYLATION_CONTEXT_LAUNCH_CHUNKS={LAUNCH_CHUNKS} "
        "not in {1, 2, 4, 8}"
    )
N_TILES = LAUNCH_CHUNKS

SEQ_CHUNK_BYTES = int(_os.environ.get(
    "BIONPU_METHYLATION_CONTEXT_SEQ_CHUNK_BYTES", "4096"))
if SEQ_CHUNK_BYTES not in (1024, 2048, 4096):
    raise ValueError(
        "BIONPU_METHYLATION_CONTEXT_SEQ_CHUNK_BYTES="
        f"{SEQ_CHUNK_BYTES} not in {{1024, 2048, 4096}}"
    )
SEQ_OVERLAP_BYTES = 4
HEADER_BYTES = 8
...
N_CHUNKS_PER_LAUNCH = int(_os.environ.get(
    "BIONPU_METHYLATION_CONTEXT_N_CHUNKS_PER_LAUNCH", "1"))
if N_CHUNKS_PER_LAUNCH not in (1, 2, 4, 8):
    raise ValueError(
        "BIONPU_METHYLATION_CONTEXT_N_CHUNKS_PER_LAUNCH="
        f"{N_CHUNKS_PER_LAUNCH} not in {{1, 2, 4, 8}}"
    )


def methylation_context(dev):
    """Build the IRON program for methylation-context scanning."""
    seq_in_chunk_bytes = SEQ_CHUNK_BYTES + SEQ_OVERLAP_BYTES
    ...
    tile_fn = Kernel(
        "methylation_context_tile",
        "methylation_context_tile.o",
        [
            seq_chunk_ty,
            partial_chunk_ty,
            np.int32,
    ...
    partial_fifos = of_sparse.prod().join(
        ...
        names=[f"partial_methylation_context_{i}" for i in range(N_TILES)],
        depths=[1] * N_TILES,
    )

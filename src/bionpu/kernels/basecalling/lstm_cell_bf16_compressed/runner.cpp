...
Args parse(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (k == "-N" || k == "--sparsity-n") a.sparsity_n = std::stoi(next());
    else if (k == "-M" || k == "--sparsity-m") a.sparsity_m = std::stoi(next());
    else if (k == "--measure-dma") a.measure_dma = true;
  ...
  if (a.sparsity_m < 2 || a.sparsity_n <= 0 || a.sparsity_n >= a.sparsity_m)
    throw std::runtime_error(
        "invalid sparsity (N, M): require 0 < N < M and M >= 2");
  return a;
}
...
ExpandedWB expand_wb(const std::vector<bf16> &compact, int L) {
  ...
  auto emit_chunk = [&](const bf16 *src_full,
                        bf16 *dst,
                        int half_idx) {
...
int main(int argc, char **argv) {
  ...
  const size_t dense_weight_bytes = wb.weight.size() * BYTES_PER_BF16;
  const size_t theoretical_compressed_bytes =
      static_cast<size_t>(static_cast<double>(dense_weight_bytes) *
                          args.sparsity_n / args.sparsity_m);

  if (args.measure_dma) {
    std::cout << "DMA dense bytes: " << dense_weight_bytes << std::endl;
    std::cout << "DMA theoretical compressed bytes ("
              << args.sparsity_n << ":" << args.sparsity_m
